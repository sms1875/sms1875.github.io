...
vector<int> solution(vector<string> id_list, vector<string> report, int k) {
    vector<int> answer(id_list.size(), 0); // 각 유저가 받은 결과 메일의 개수를 저장할 벡터 (초기값 0으로 설정)
    map<string, vector<string>> m; // 각 유저가 신고한 유저 리스트를 저장하는 맵
    map<string, int> reportmap; // 각 유저가 신고당한 횟수를 저장하는 맵

    // 중복 신고 제거 (report 리스트 정렬 후 중복 제거)
    sort(report.begin(), report.end());
    report.erase(unique(report.begin(), report.end()), report.end());

    // 신고 정보를 맵에 저장
    for (const auto& rp : report) {
        int space = rp.find(" "); // " " (공백) 위치 찾기
        string a = rp.substr(0, space); // 신고자 ID 추출
        string b = rp.substr(space + 1, rp.size() - 1); // 신고당한 ID 추출
        m[a].emplace_back(b); // 신고자가 신고한 유저 리스트에 추가
        reportmap[b]++; // 신고당한 유저의 신고 횟수 증가
    }

    // 각 유저가 신고한 유저 중에서 정지된 유저의 수를 계산
    for (int i = 0; i < id_list.size(); i++) {
        int sum = 0; // 각 유저가 신고한 유저 중 정지된 유저 수
        for (auto id : m[id_list[i]]) { // 해당 유저가 신고한 유저 리스트 순회
            if (reportmap[id] >= k) // 신고 횟수가 k 이상인 경우 정지된 것으로 간주
                sum++;
        }
        answer[i] = sum; // 해당 유저의 결과 메일 개수 저장
    }
