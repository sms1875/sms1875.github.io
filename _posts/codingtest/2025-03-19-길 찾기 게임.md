---
layout: post
title: "[3] 길 찾기 게임 - 42892"
date: 2025-03-19 13:26:01+0900
...
class Solution {
    ...
    public int[][] solution(int[][] nodeinfo) {
        ...
        // 노드 리스트에 추가 (번호는 1부터 시작)
        for (int i = 0; i < n; i++) {
            nodes.add(new Node(i + 1, nodeinfo[i][0], nodeinfo[i][1]));
        }

        // **y 기준 내림차순, y가 같으면 x 기준 오름차순 정렬**
        nodes.sort((a, b) -> b.y == a.y ? a.x - b.x : b.y - a.y);

        // **트리 구성**
        Node root = nodes.get(0); // 가장 높은 y 값이 루트
        for (int i = 1; i < n; i++) {
