...
vector<int> solution(vector<int> fees, vector<string> records) {
    ...
    map<string, bool> checkOut;
    vector<int> answer;
    for(auto record: records){
        vector<string> recArr = split(record,' ');
        
        string carNum = recArr[1];
        
        vector<string> time = split(recArr[0],':');
        
        int hour = stoi(time[0]);
        int minute = stoi(time[1]);
        
        if(recArr[2] == "IN") {
            timeMap[carNum] -= (hour*60 + minute);
            checkOut[carNum] = true;
        }
        else {
            timeMap[carNum] += (hour*60 + minute);
            checkOut[carNum] = false;
        }
    }
    
    for(auto isOut : checkOut){
        if(isOut.second){
            timeMap[isOut.first] += (23*60 + 59);
        }
    }
    
    for(auto time : timeMap){
        if(time.second < fees[0]) answer.push_back(fees[1]);
        else {
            int overTime = time.second - fees[0];
            int a = (overTime)%fees[2]? 1:0;
            int fee = fees[1] + ((overTime)/fees[2] + a) * fees[3];
